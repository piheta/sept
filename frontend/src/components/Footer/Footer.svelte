<script lang="ts">
  ...
  function toggleShowGifBox() {
    showGifBox = !showGifBox;
  }
  function toggleShowEmoteBox() {
    showEmoteBox = !showEmoteBox;
  }
  ...
  function sendMessage(message, chat_id) {
    ...
    SendMessage(message, chat_id)
      .then((allMessagesInChat: models.Message[]) => {
  ...
  function adjustHeight() {
    ...
    const targetHeight =
      originalHeight + Math.max(0, newLines - 1) * lineHeight;
...
{#if showGifBox}
  <GifBox
    mb={height}
    chat_id={$selection_store.id}
    {sendMessage}
    toggleShow={toggleShowGifBox}
  />
{/if}
{#if showEmoteBox}
  <EmoteBox
    ...
    chat_id={$selection_store.id}
    {sendMessage}
    toggleShow={toggleShowEmoteBox}
  />
{/if}

<div
  class="w-full rounded-md p-2 flex flex-col shadow-xl scrollbar-chatbox"
  style="height:{height}px; background-color: rgba(17, 24, 39, 0.5);"
>
  <div class="flex -ml-0.5 *:mr-1 mb-0.5">
    ...
    <svg
      xmlns="http://www.w3.org/2000/svg"
      class="-ml-0.5 hover:bg-gray-200 hover:bg-opacity-5"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
      fill="currentColor"
    ...
    <button on:click={toggleShowEmoteBox}
      ><svg
        xmlns="http://www.w3.org/2000/svg"
        width="1em"
        height="1em"
        ...
        ><g
          ...
          ><circle cx="12" cy="12" r="10" /><path
            d="M8 14s1.5 2 4 2s4-2 4-2M9 9h.01M15 9h.01"
          /></g
        ></svg
      ></button
    >
    <button on:click={toggleShowGifBox}
      ><svg
        xmlns="http://www.w3.org/2000/svg"
