<script lang="ts">
    ...
    let { height = $bindable() }: Props = $props();
    let input_txt = $state("");
    ...
    let showGifBox = $state(false);
    let showEmoteBox = $state(false);
    ...
    run(() => {
        adjustHeight();
    }); // Recalculate height whenever input_txt or height changes
</script>
