<script lang="ts">
  ...
  const emoteImports = import.meta.glob("../../assets/emotes/*.webp", {
    eager: true,
  });
  ...
  const sendEmote = (url) => {
    sendMessage(
      `<img class="mt-1.5 mb-1 rounded-md max-h-10 max-w-15" src="${url}" alt="gif" />`,
      chat_id,
    );
    toggleShow(false);
  };
...
<div
  class="h-64 w-72 p-2 rounded-md shadow-lg absolute left-[40vw]"
  style="bottom:calc({mb}px + 1rem); background-color: rgba(17, 24, 39, 0.98);"
>
  ...
  <div
    class="overflow-y-auto w-full grid grid-cols-[repeat(auto-fill,minmax(1.5rem,1fr))] gap-1 no-scrollbar"
  >
