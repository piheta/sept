<script lang="ts">
  ...
  function sendGif(giflink) {
    sendMessage(
      `<img class="mt-1.5 mb-1 rounded-md max-h-72 max-w-72 w-5/6" src="${giflink}" alt="gif" />`,
      chat_id,
    );
    toggleShow();
  }
...
>
  ...
  <div
    class="overflow-y-auto w-full relative h-[calc(100%-2.5rem)] flex gap-2 no-scrollbar"
  >
    <div class="w-1/2">
