...
{#if small}
  <SmallList items={chats} />
{:else}
  <button
    on:click={toggleShow}
    class="w-full flex items-center align-middle text-center mt-0 pt-0 pb-1 pl-3 cursor-pointer group"
  >
    <span
      class="inline-block transition-transform duration-150 ease-in-out"
      class:rotate-0={showChats}
      class:-rotate-90={!showChats}
    >
      <svg
    ...
    <p class="text-md inline-block ml-1">Friends</p>

    <span
      on:click={fillSearchBoxAdd}
      class="ml-auto w-5 transition-opacity duration-150 ease-in-out opacity-0 group-hover:opacity-100"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="1em"
    ...
    <span
      class="mr-1 w-5 transition-opacity duration-150 ease-in-out opacity-0 group-hover:opacity-100"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="1.2em"
  ...
  <List items={chats} bind:showItems={showChats} />
{/if}
