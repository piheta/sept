...
{:else}
            <div
                class="w-full flex items-center align-middle text-center mt-2 pt-0 pr-20 pb-1 pl-2 select-none"
            >
                <p class="text-lg inline-block ml-1">
                    {title}
                </p>
    ...
    <ul class="cursor-pointer" use:dndzone={{items, flipDurationMs, dropTargetStyle}} on:consider={handleDndConsider} on:finalize={handleDndFinalize}>
        {#if showItems}
