...
<div class="flex flex-col h-full">
    <div class="flex-shrink-0">
        ...
        >
            <button onclick={() => {sidebar_mode = 0; searchQuery=""}} class:!mt-2={small} class="relative w-full h-12 rounded-md flex items-center justify-center cursor-pointer">
            ...
            <button onclick={() => {sidebar_mode = 1; searchQuery=""}} class="relative w-full h-12 rounded-md flex items-center justify-center cursor-pointer">
            ...
            <button onclick={() => {sidebar_mode = 2; searchQuery=""}} class:hidden={small} class="relative w-full h-12 rounded-md flex items-center justify-center cursor-pointer">
            ...
            <button onclick={() => {sidebar_mode = 0;}} class:hidden={!small} class="relative w-full h-12 rounded-md flex items-center justify-center cursor-pointer">
    ...
    <div class="overflow-y-auto flex-grow no-scrollbar">
        {#if sidebar_mode === 0}
            <SidebarUsers small={small} fillSearchBoxAdd={fillSearchBoxAdd}/>
        ...
        {:else if sidebar_mode === 3}
            ...
            {#if foundUser}
                <div class="mt-1 h-[34px] items-center text-[0.9rem] p-[0.3rem] m-0 rounded-md focus:list-none focus:outline-none outline-none hover:bg-[rgba(0,0,0,0.125)] focus:bg-[rgba(0,0,0,0.125)] ml-2 flex"> 
                    ...
                    {foundUser.username}
                    <button onclick={() => {SendOffer(foundUser.ip)}} class="ml-auto hover:cursor-pointer"><svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1.25em" viewBox="0 0 24 24" fill="currentColor"><path d="M11 11V7H13V11H17V13H13V17H11V13H7V11H11ZM12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22ZM12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20Z"></path></svg></button>
                </div>
            {:else}
